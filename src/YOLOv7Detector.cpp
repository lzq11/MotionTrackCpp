...
std::vector<Detection> YOLOv7Detector::detect(cv::Mat &image)
{
    ...
    for (int32_t i = 0; i < num_detections; i++)
    {
        ...
        if (detection.box.y < 0)
            detection.box.y = 0;
        if (detection.box.x + detection.box.width > image.cols)
            detection.box.width = image.cols-detection.box.x;
        if (detection.box.y + detection.box.height > image.rows)
            detection.box.height = image.rows-detection.box.y;
    }
...
//     cmd.add<std::string>("model_path", 'm', "Path to onnx model.", true, "yolov7.onnx");
...
//         return -1;
//     }
//     YOLOv7Detector yolo_detector(modelPath.c_str(), cmd.exist("int8"), cmd.exist("fp16"));
