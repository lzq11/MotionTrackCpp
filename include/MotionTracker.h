...
{
public:
	MotionTracker(int frame_rate = 30, int track_buffer = 30);
	~MotionTracker();
...
private:
	...
	void fuse_classes_width_height(vector<vector<float> > &cost_matrix,vector<STrack*> &atracks, vector<STrack> &btracks);
	
	vector<vector<float> > motion_distance(vector<STrack*> &atracks, vector<STrack> &btracks, int motion_thresh,int &dist_size, int &dist_size_size);

	void index_detection(vector<STrack> &detections , vector<int> &u_detection);
	
	void index_strack_pool(vector<STrack*> &strack_pool , vector<int> &u_track);
	double lapjv(const vector<vector<float> > &cost, vector<int> &rowsol, vector<int> &colsol, bool extend_cost = false, float cost_limit = LONG_MAX, bool return_cost = true);
